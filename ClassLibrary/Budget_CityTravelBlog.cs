...
namespace BudgetCBL
{
    public partial class Budget_CityTravelBlog
    {
        ...
        public Budget_CityTravelBlog()
        {
        }
        public Budget_CityTravelBlog(string sConnectionString)
        {
            ConnectionString = sConnectionString;
        }

        public List<Budget_CityTravelBlog> SelectAll()
        {
            List<Budget_CityTravelBlog> TravelBlogList = null;
            try
            {
                string sSQL = "SELECT * FROM CityTravelBlog";

                DataSet ds = SqlHelper.ExecuteDataset(ConnectionString, CommandType.Text, sSQL);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    TravelBlogList = new List<Budget_CityTravelBlog>();
                    Budget_CityTravelBlog blog = null;
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        blog = new Budget_CityTravelBlog();
        ...
        public Budget_CityTravelBlog SelectByOID(Guid guidTravelBlogOID)
        {
            Budget_CityTravelBlog blog = null;
            try
            {
                string sSQL = "SELECT * FROM CityTravelBlog WHERE TravelBlogOID=@TravelBlogOID";
        ...
        public bool add()
        {
            ...
            try
            {
                string sSQL = @"INSERT INTO CityTravelBlog 
        ...
        public bool update()
        {
            ...
            try
            {
                string sSQL = @"UPDATE CityTravelBlog SET 
